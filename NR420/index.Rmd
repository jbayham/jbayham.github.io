...
- First, create a folder for the lab in a directory that you can edit.  
- Rename the new folder `CV_Lab`.  
- Create a new folder inside of `CV_Lab` called `data`.  

You can download the data at: [practice data](https://jbayham.github.io/NR420/data/practice_data.csv)
...
## Exploring the Data
It is always a good idea to explore the data before estimating any models.  You can view the first several rows of the data frame using the command `str()`.
...
Let's plot the relationship between the bid amount and the proportion of respondents saying "yes" to paying the fee.  We can use the `dplyr` package to group the data by bid amount and calculate the mean response for each bid level.  Then we can plot the results using `ggplot2`.
```{r plot_bid, message=FALSE, warning=FALSE}
#Group bids in .5 increments
practice.data %>%
  mutate(BID=round(BID/.5)*.5) %>%
  group_by(BID) %>%
  summarize(prop.yes=mean(y)) %>%
  ggplot(aes(x=BID,y=prop.yes)) +
  geom_point() +
  geom_smooth(method="lm",se=FALSE) +
  #geom_smooth(method="glm", method.args = list(family="binomial")) +
  scale_y_continuous(labels = scales::percent,limits = c(0,1)) +
  scale_x_continuous(breaks = seq(0 , 7 , 1 ), labels = dollar) +
  coord_flip() +
  labs(title="Proportion of Yes Responses by Bid Amount", x="Bid Amount", y="Proportion Yes")
...
You can download the dataset here: [Loomis data](https://jbayham.github.io/NR420/data/loomis_data.csv)  
